...
function rowspanAndColspanInFooter({
  rowIndex,
  colIndex,
}: {
  rowIndex: BaseTableCellParams<DataType>['rowIndex'];
  colIndex: BaseTableCellParams<DataType>['colIndex'];
}) {
  // 中间列合并，收尾两列不合并
...
function rowClassName({ type }: { type: 'body' | 'foot' }) {
