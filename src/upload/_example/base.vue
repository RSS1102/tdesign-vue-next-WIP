...
import { MessagePlugin, ProgressContext, SuccessContext, UploadFile, UploadValidateType } from 'tdesign-vue-next';
...
const handleFail = ({ file }: { file: UploadFile }) => {
  MessagePlugin.error(`文件 ${file.name} 上传失败`);
};

const handleSelectChange = (files: { files: File[] }) => {
  console.log('onSelectChange', files);
};

const handleSuccess = (params: SuccessContext) => {
  console.log('success', params);
  MessagePlugin.success('上传成功');
};

// 多文件上传，一个文件一个请求场景，每个文件也会单独触发上传成功的事件
const onOneFileSuccess = (params: Pick<SuccessContext, 'e' | 'file' | 'response' | 'XMLHttpRequest'>) => {
...
const handleProgress = (params: ProgressContext) => {
